...
- **Q2 — Fan-out snapshot policy.** Evaluated in [RFC 0004: Multi-Consumer Fan-Out Heddles](rfcs/0004-fanout-heddles.md). 1-writer, N-reader seqlock ring preserves Law 2 zero allocations (1.27M pub/s across 4 consumers). Implemented in the TS driver layer (`@weft/core`) and — byte-compatible across ports, proven by `fixtures/xlang-fanout/` — in the canonical languages as sibling driver-layer modules (`core/c/fanout.{h,c}`, `core/rust/src/fanout.rs`; exhaustive Loom model + TSAN/ASAN/torture gates in `litmus/evidence/fanout/`); the same C ring now also serves Android (JNI surface in `weft_jni.c`, plus the pure-JVM semantics port `core/kotlin/Fanout.kt`) and Flutter (Dart FFI in `packages/flutter_weft`, with cross-isolate consumers); the kernel Triad stays 1:1 and frozen.
