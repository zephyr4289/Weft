#!/usr/bin/env bash
# run_weft_cluster_shard.sh — RFC-0019: the kernel-bypass cluster shard
# (Pillar 3 / weft-cluster).
#
...
# --- 0. environment ---------------------------------------------------------
step "environment"
{ uname -a; gcc --version | head -1; grep CapEff /proc/self/status; } | tee -a "$LOG"
...
for t in test-rdma test-xdp test-uring test-fabric; do
    if ./$t > "$ROOT/$EV/cl-${t#test-}.log" 2>&1; then
        echo "$t: ok ($(grep -c '\[PASS\]' "$ROOT/$EV/cl-${t#test-}.log") gates)" | tee -a "$ROOT/$LOG"
    else
...
if timeout 120 tools/weft-cluster/weft-cluster-bench \
    ...
    echo "bench: PASS ($(grep verdict "$EV/cl-bench.log"))" | tee -a "$LOG"
else
...
step "verdict"
if [ "$fail" -eq 0 ]; then
    echo "shard-weft-cluster: GREEN" | tee -a "$LOG"
else
    echo "shard-weft-cluster: RED" | tee -a "$LOG"
fi
exit "$fail"
