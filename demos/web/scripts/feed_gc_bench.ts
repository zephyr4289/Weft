...
emit('- Mode C: 8.9 KB/window vs B\u0027s 144 B isolates, by subtraction, the kernel TS');
emit('  port\u0027s per-publish advisory telemetry — BigInt atomics (t_publish, t_wsteps,');
emit('  BigInt(seq) canary) at ~90 B/publish, a declared TS-port cost (06-PITFALLS \u00a74');
emit('  "safe but noisier"), not a W6-engine cost. P+C adds the consume path: one');
emit('  rReadSlice view per claim (kernel public read API) + <= 3 cached Float32');
emit('  wrappers total — the W1-W5 ModeCRunner pays two views per frame.');
