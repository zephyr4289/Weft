...
emit('  publish+claim cycle and a ~1.5x throughput improvement. P+C once added the');
emit('  consume path on top: one rReadSlice view object per claim (W6 runner) or');
emit('  two view objects per frame (W1-W5 ModeCRunner: rReadSlice + a fresh');
emit('  Float32Array wrapper) \u2014 ~20.9 KB/window of runner-side allocation. The');
emit('  2026-09-16 consume-path fix closed it: both runners now claim once and');
emit('  read through rLiveFloat32() \u2014 the kernel\u0027s per-slot CACHED live view,');
emit('  the documented draw-phase API (TS analog of C\u0027s weft_r_live_ptr).');
emit('  P+C now measures 144 B/window for ALL modes: B == C == D at the sampling-');
emit('  noise floor end-to-end \u2014 produce AND consume, Law 2 closed through the');
emit('  whole frame path.');
