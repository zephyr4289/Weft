...
> **v1.1 (amended by WO-P0A, 2026-09-11):** §5 negotiation-table row 3 corrected — `(W=2, S={1}) → 1`, not `BIND_INCOMPATIBLE`. The v1.0 table contradicted itself (row 4 requires downgrade capability; row 3 denied it); the §3 formula is the only self-consistent reading and is confirmed normative. §3 gains the writer-ceiling commitment. Set-valued generalization filed as RFC-0002 (deferred until a second version exists).
...
**Writer-ceiling commitment (v1.1):** `W` is a *capability ceiling*. A writer declaring `W` vouches that it can emit every version `v ≤ W` for this Weft's lifetime. This is coherent with the additive evolution mechanism (§4): a v2 writer emits a v1 frame simply by writing `header_size=16` envelopes — old layout minus unknown trailing fields. A future writer that *cannot* honestly advertise the full ceiling (semantic version break, §4 line 2) must not use the scalar form; that is exactly the gap RFC-0002's capability sets close. Until v2 exists this is moot: every Phase 0 writer declares `W=1` and binds `v=1`.
...
| negotiation table | (W=1, S={1})→1 · (W=2, S={1,2})→2 · (W=2, S={1})→**1** *(v1.1: was BIND_INCOMPATIBLE — typo; the §3 formula is normative, and row 4 already presupposes downgrade capability, so row 3 denying it made the table self-contradictory)* · (W=3, S={1,2})→2 |
