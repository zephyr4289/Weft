...
| L7 `poison_intact=false` or TSAN race on buffer bytes | Three causes, triage in order: (1) poison ran before ACK — kernel bug (A1); (2) kernel wrote bytes post-ACK — kernel bug; (3) **caller wrote payload after receiving `PUB_DROPPED_REVOKED`** — caller/harness bug (found by TSAN 2026-09-11: harness `fill_payload` before `publish` kept writing post-ACK). All three are STOP-and-file; only (3) fixes in the harness. The same-iteration window (`fill_payload` → `publish` that ACKs) is race-free by construction: fill → ACK `fetch_add(AcqRel)` → reclaim Acquire poll → poison is a complete happens-before chain. | 02 §6, WO-P0A §11 |
