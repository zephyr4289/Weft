...
## 11. Post-sign-off addendum (2026-09-11): Phase 0 CLOSED, TSAN finding adjudicated

**Phase 0 sign-off ACCEPTED.** G1 + G2 (24/24, v1.1 predicates) + G3 (TSAN clean ×5 after the L7 fix) all met. Catalog v2, REPORT v2 with filled sign-off, v1 preserved. Well executed.

**The TSAN-caught L7 race — ruling.** Root cause was **harness-side, not kernel-side**: the runner's writer loop called `fill_payload` before every `publish`; after `publish` returned `PUB_DROPPED_REVOKED` (ACK issued at top of publish), the *next* iteration's `fill_payload` wrote buffer bytes post-ACK, racing reclaim's poison `memset`. The fix (skip `fill_payload` once `DROPPED_REVOKED` is seen, applied to C and Rust, mirroring the TS pattern) is **correct and sufficient**.

Two consequences, both already applied:

1. **Caller contract made explicit** (02 §6 in spirit, 06 §5 registry updated): `w_publish` returning `PUB_DROPPED_REVOKED` is the I6 handshake's release edge **to the caller** — after receiving it, the caller MUST NOT touch buffer bytes again. Future harness/authors get this as a listed pitfall, not folklore.
2. **My v1.0 registry line was wrong** — it claimed an L7 post-ACK write could never be a harness bug. This finding refutes that; the registry now lists three causes with triage order (kernel-poison-early / kernel-post-ACK-write / caller-post-ACK-write). The residual same-iteration window (`fill_payload` → `publish`-that-ACKs) is race-free **by construction**: fill → ACK `fetch_add(AcqRel)` → reclaim Acquire poll → poison forms a complete happens-before chain. No kernel change is needed or permitted.

**Sequencing (decision):** Phase 0.5 — L-loom — runs **first** (half day, de-risks the formal claim before any performance number is quotable publicly; RFC-0001 flips to Accepted on loom evidence), then **Phase 1 per `WO-P1-BENCHMARKS.md`**.
