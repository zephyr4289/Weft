...
- **Built (VM ports + flight recorder, 2026-09-16, Series 5)**: Kotlin (`core/kotlin/Fanout.kt` — VarHandle fenced acq/rel over one direct ByteBuffer; F1–F10 battery incl. a real 100k-frame writer+3-reader thread torture), Swift (`core/swift/Fanout.swift` — UnsafeAtomic SC stamps + relaxed-atomic payload words over one 64-byte-aligned region; F1–F10 incl. DispatchQueue torture), and Dart (`core/dart/fanout.dart` — single-isolate reference with FFI attach, the port's honesty wall) — all three byte-compatible, all mirrored into their package twins (binding-parity hashed), memory-model mappings normative in `docs/PORTS.md` §6, structurally enforced by the port validator's fan-out rule pack. Plus the flight recorder this RFC's motivation named: `tools/weft-fanout-rec` (.weftrec v2, `tools/FORMATS.md` §1.5) — shm-attached capture with honest per-claim drop accounting, CRC + telescoping validation, content-faithful replay; selftest green across both C ordering regimes, ASAN, and TSAN (`litmus/evidence/fanout/flight-recorder.log`), gated as step 5 of the `fanout-native` CI shard.
