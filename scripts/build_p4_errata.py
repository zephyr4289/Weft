#!/usr/bin/env python3
"""
build_p4_errata.py — Phase 5 / T0 / C3 Phase 4 Errata PDF.

Per WO-P4-C1R-VERIFICATION §3 + §4: C3 is the Phase 4 errata document,
absorbing:
  - E-1: v1.0.4 changelog typo (C2-W2 → C1-W2; senior's errata finding)
  - B1 claim-history correction (v1.0.2 false → v1.0.3 four defects → v1.0.4 repair)
  - B2 soak evidence pointer (World A, closed at C2)
  - B3 verification capture run params (hz=120 payload=64 secs=30 per C2)
  - T1 Swift-row column overlap (will be re-typeset with breakable token)
"""
import hashlib
import shutil
import subprocess
...
OUT_MD = OUT_DIR / "Weft-Phase4-Errata.md"
OUT_PDF = OUT_DIR / "Weft-Phase4-Errata.pdf"

MD = """# Weft Phase 4 Errata — Closure Batch C3
...
# Typeset PDF via pandoc + tectonic (same XeTeX-native template as v1.0.4 + C2)
WORKDIR = Path("/home/z/my-project/scripts/_c3_build")
...
# Verify the T1 Swift-row no longer overruns col 2 boundary (x1 > 392pt)
print("\n→ T1 Swift-row column overlap check (post-fix):")
flagged = 0
    ...
    for blk in page.get_text("dict")["blocks"]:
        ...
        for line in blk.get("lines", []):
            for span in line.get("spans", []):
                t = span["text"]
                if "ManagedAtomic" in t:
                    x0, x1 = span["bbox"][0], span["bbox"][2]
                    overrun = x1 - 392.0
                    print(f"  p{pno+1} x0={x0:.1f} x1={x1:.1f} overrun={overrun:.1f}pt  {t!r}")
                    if overrun > 0:
                        flagged += 1
print(f"  Swift-row col 2 overruns remaining: {flagged} (target: 0)")
