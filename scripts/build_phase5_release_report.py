#!/usr/bin/env python3
"""
build_phase5_release_report.py — Phase 5 / T8: Weft-Phase5-Release-Report.pdf.

Per WO-P5-RELEASE §1.T8. Contents:
  - T0 batch evidence recap
  - W-suite design notes (fairness pin mechanics, D-vs-C findings published honestly)
  - Thermal honesty label
  - Site structure
  - Clean-tree validation log summary
  - SHA256SUMS listing
  - Known limitations (what the sandbox cannot prove — all Phase 6+)
  - Deviations field (mandatory)
  - Verification claims with scope + threshold
"""
import hashlib
import shutil
import subprocess
...
OUT_MD = OUT_DIR / "Weft-Phase5-Release-Report.md"
OUT_PDF = OUT_DIR / "Weft-Phase5-Release-Report.pdf"

MD = """# Weft Phase 5 — Sandbox Release Report
...
# Typeset PDF via pandoc + tectonic (same XeTeX-native template as v1.0.4 + C2 + C3)
WORKDIR = Path("/home/z/my-project/scripts/_phase5_report_build")
WORKDIR.mkdir(parents=True, exist_ok=True)

# Reuse the C3 template (it's the same XeTeX-native format)
