#!/usr/bin/env python3
"""
build_phase5_release_report_r5.py — Phase 5 / R5: Re-typeset release report v1.0.1.

Per WO-P5-VERIFICATION §4 R5 + §3 R-E:
  - No PAST-CROPBOX flags (state scope+threshold per WO-P4-CLOSURE P4-W1)
  - Rewritten deviation entries:
    - D-T7-1 → true exposure-shortfall account + retry implementation (R2)
    - D-T7-2 → closed by Path C (R1)
    - NEW: D-T7-3 (P5-W1): v1.0.3 historical mutation — staff-provided original BLOCKED
    - NEW: D-T7-4 (P5-W3): C3 errata §3 misattribution — R4 correction slip delivered
  - Sign-off lines complete: 'pending C5r' → 'pending Phase 5 staff review'
  - T0/T7 checklist updated to R-batch outcomes
  - Tarball hash externalized (breaks the chicken-and-egg loop)
"""
import hashlib
import shutil
import subprocess
...
OUT_MD = OUT_DIR / "Weft-Phase5-Release-Report-v1.0.1.md"
OUT_PDF = OUT_DIR / "Weft-Phase5-Release-Report-v1.0.1.pdf"

# Compute the current tarball hash to record it in the report
TARBALL = OUT_DIR / "weft-sandbox-v0.1.tar.gz"
TARBALL_SHA = hashlib.sha256(TARBALL.read_bytes()).hexdigest() if TARBALL.exists() else "<not built>"

MD = f"""# Weft Phase 5 — Sandbox Release Report v1.0.1
...
# Typeset PDF via pandoc + tectonic (XeTeX-native template — breakable hash strings this time)
WORKDIR = Path("/home/z/my-project/scripts/_phase5_report_r5_build")
WORKDIR.mkdir(parents=True, exist_ok=True)

# Reuse the XeTeX-native template, but with widened textwidth + smaller mono font
# to ensure hash strings wrap or shrink instead of clipping
TEMPLATE_SRC = Path("/home/z/my-project/scripts/_c3_build/template.tex")
TEMPLATE_FILE = WORKDIR / "template.tex"
shutil.copy(TEMPLATE_SRC, TEMPLATE_FILE)

# Patch the template: shrink mono font + use sloppy for line breaks
import re
template_text = TEMPLATE_FILE.read_text()
# Reduce mono font scale + add sloppypar for verbatim blocks
template_text = template_text.replace(
    r"\setmonofont{Liberation Mono}",
    r"\setmonofont[Scale=0.78]{Liberation Mono}"
)
# Force the verbatim environment to wrap by setting a smaller line width
template_text = template_text.replace(
    r"\DefineVerbatimEnvironment{Highlighting}{Verbatim}{commandchars=\\\{\}}",
    r"\DefineVerbatimEnvironment{Highlighting}{Verbatim}{commandchars=\\\{\},fontsize=\small,breaklines=true,breakanywhere=true}"
)
TEMPLATE_FILE.write_text(template_text)

tex_path = WORKDIR / "phase5_v1.0.1.tex"
pandoc_cmd = [
    "pandoc",
...
produced = WORKDIR / "phase5_v1.0.1.pdf"
...
# R5 verification: scan for PAST-CROPBOX flags at 611.5pt threshold
print("\n→ R5 verification: PAST-CROPBOX scan at 611.5pt threshold (whole document)")
THRESH = 611.5
flagged = 0
    ...
    for blk in page.get_text("dict")["blocks"]:
        ...
        for line in blk.get("lines", []):
            for span in line.get("spans", []):
                ...
                if x1 > THRESH:
                    flagged += 1
                    print(f"  p{pno+1} x1={x1:.1f}/{pw:.1f}  PAST-CROPBOX  {text!r}")
print(f"  Flagged (x1 > {THRESH}pt): {flagged}")
print(f"  Scope: whole document, all {doc.page_count} pages")
print(f"  Method: PyMuPDF {fitz.__version__}")
if flagged > 0:
    print(f"  !! {flagged} PAST-CROPBOX lines remain — R5 release gate FAILS")
    sys.exit(2)
print(f"  R5 v1.0.1 scan-clean (0 flagged lines).")
