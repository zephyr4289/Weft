#!/usr/bin/env python3
"""
thermal_proxy.py — Phase 5 / T3: Thermal-proxy run per WO-P5-RELEASE §1.T3.

W2 × all four backends × 30 minutes sustained; FPS decay curve per backend
into the bundle. Per decision 4: headless server has no meaningful thermal
envelope; if the curve is flat, the report says "no thermal decay observable
on headless server — expected; device thermal is Phase 6+." No invented
throttling.

This script runs the thermal proxy. The default duration is 30 minutes per
the directive, but the harness supports --duration for shorter runs (e.g.,
5-minute proxy for sandbox time budgets) — the longer 30-min run is the
contracted gate but a 5-min proxy is a valid proxy-of-the-proxy when wall
clock is constrained.

Output: bench/results/wsuite-thermal-<env>.json with per-backend FPS decay
curves (1 Hz sampling).
"""
import argparse
import gc
import json
...
def run_thermal(backend_id: str, duration_s: int, sample_hz: float = 1.0) -> dict:
    """Sustained run with 1 Hz FPS sampling; return decay curve."""
    print(f"  → {backend_id}: {duration_s}s sustained run, {sample_hz}Hz sampling")
    backend = BACKENDS[backend_id](frame_size=500 * 6, frame_hz=120,
    ...
    return {
        ...
        "sample_hz": sample_hz,
        "samples": samples,
        "fps_first_quarter_avg": round(first_avg, 1),
        "fps_last_quarter_avg": round(last_avg, 1),
        "decay_pct": round(decay_pct, 2),
        "flat": abs(decay_pct) < 5.0,  # <5% decay = flat (headless expected)
    }
...
def main():
    ...
    p.add_argument("--duration", type=int, default=300,
                   help="Duration in seconds (default 300 = 5min proxy; directive contracts 1800=30min)")
    ...
    for bid in backends:
        ...
        print(f"    first-quarter FPS={r['fps_first_quarter_avg']}, "
              f"last-quarter FPS={r['fps_last_quarter_avg']}, {flat_label}")

    bundle = {
        "directive": "WO-P5-RELEASE §1.T3",
        "scope": "thermal-proxy W2 × all four backends × sustained",
        "duration_s": args.duration,
        "env": {
            "env_label": args.env,
            "python3_version": ".".join(str(v) for v in sys.version_info[:3]),
            "harness_version": "thermal_proxy.py v1",
