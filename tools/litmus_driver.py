...
def main():
    ...
    # Assemble the matrix
    write_report(catalog, langs, results, raw)
    write_results_json(catalog, langs, results, raw)
...
def write_report(catalog, langs, results, raw):
    ...
    lines.append("")
    lines.append("```")
    lines.append("Phase 0 status: 20/24 cells green (plus 4 documented findings).")
...
def write_results_json(catalog, langs, results, raw):
    import datetime
    bundle = {
