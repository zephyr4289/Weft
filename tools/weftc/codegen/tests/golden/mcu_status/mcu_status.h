// mcu_status.h — generated by weftc-codegen 0.1.0 (target: c). DO NOT EDIT.
...
#ifndef WEFT_MCU_STATUS_H
#define WEFT_MCU_STATUS_H

#define WEFT_MCU_STATUS_SCHEMA_ID UINT64_C(0xCDB73E6BFDEC4F07)
#define WEFT_MCU_STATUS_SIZE 32u
#define WEFT_MCU_STATUS_ALIGN 8u
#define WEFT_MCU_STATUS_OFF_SCHEMA_ID 0u
#define WEFT_MCU_STATUS_OFF_FLAGS 8u
#define WEFT_MCU_STATUS_OFF_VBUS_MV 12u
#define WEFT_MCU_STATUS_OFF_TEMP_C 14u
#define WEFT_MCU_STATUS_OFF_GYRO_TEMP_C 16u
#define WEFT_MCU_STATUS_OFF_MOTOR_RPM 20u
#define WEFT_MCU_STATUS_LEN_MOTOR_RPM 4u
#define WEFT_MCU_STATUS_OFF_CRC32 28u

#ifdef __cplusplus
extern "C" {
#endif

/// weft_mcu_status_t — 32 bytes, align 8. Offsets are normative (weft-ir),
/// synthesized padding members make the C declaration match them exactly.
/// Embedded MCU heartbeat: bitfields, f16 sensors, u16 arrays. CPU-rich; GPU targets refuse it (Law 4 fixture).
typedef struct weft_mcu_status {
    uint64_t schema_id;              // @0 (8)
    uint32_t flags;              // @8 (4)
    uint16_t vbus_mv; // bus voltage, millivolts              // @12 (2)
    weft_f16_t temp_c; // board temperature              // @14 (2)
    weft_f16_t gyro_temp_c;              // @16 (2)
    uint16_t weft_pad0[1];            // @18 (2)
    uint16_t motor_rpm[4]; // per-motor RPM              // @20 (8)
    uint32_t crc32;              // @28 (4)
} weft_mcu_status_t;

// Law 4 (host side): compile-time ABI guards.
static_assert(sizeof(weft_mcu_status_t) == 32, "weft ABI layout violation: mcu_status size");
static_assert(offsetof(weft_mcu_status_t, schema_id) == 0, "weft ABI: mcu_status.schema_id offset mismatch");
static_assert(offsetof(weft_mcu_status_t, flags) == 8, "weft ABI: mcu_status.flags offset mismatch");
static_assert(offsetof(weft_mcu_status_t, vbus_mv) == 12, "weft ABI: mcu_status.vbus_mv offset mismatch");
static_assert(offsetof(weft_mcu_status_t, temp_c) == 14, "weft ABI: mcu_status.temp_c offset mismatch");
static_assert(offsetof(weft_mcu_status_t, gyro_temp_c) == 16, "weft ABI: mcu_status.gyro_temp_c offset mismatch");
static_assert(offsetof(weft_mcu_status_t, motor_rpm) == 20, "weft ABI: mcu_status.motor_rpm offset mismatch");
static_assert(offsetof(weft_mcu_status_t, crc32) == 28, "weft ABI: mcu_status.crc32 offset mismatch");
...
static inline const weft_mcu_status_t* weft_cast_mcu_status(const void* buffer, size_t len, weft_error_t* err)
{
    weft_error_t rc = WEFT_OK;
    const weft_mcu_status_t* out = NULL;
    const uint8_t* p = (const uint8_t*)buffer;
    if (len < (size_t)WEFT_MCU_STATUS_SIZE) {
        rc = WEFT_ERR_SHORT_BUFFER;
    } else if ((((uintptr_t)p) & (WEFT_MCU_STATUS_ALIGN - 1u)) != 0u) {
        rc = WEFT_ERR_BAD_ALIGN;
    } else if (*(const uint64_t*)(const void*)p != (uint64_t)WEFT_MCU_STATUS_SCHEMA_ID) {
        rc = WEFT_ERR_SCHEMA_MISMATCH;
    } else {
        out = (const weft_mcu_status_t*)p;
    }
    if (err) { *err = rc; }
    return out;
}

/// Mutable twin of weft_cast_mcu_status (writer side: UMEM/DMA ring slots).
static inline weft_mcu_status_t* weft_cast_mcu_status_mut(void* buffer, size_t len, weft_error_t* err)
{
    return (weft_mcu_status_t*)(void*)(uintptr_t)weft_cast_mcu_status((const void*)buffer, len, err);
}
...
static inline weft_error_t weft_mcu_status_read_packed(const void* buffer, size_t len, weft_mcu_status_t* out)
{
    if (len < (size_t)WEFT_MCU_STATUS_SIZE) { return WEFT_ERR_SHORT_BUFFER; }
    const uint8_t* p = (const uint8_t*)buffer;
    out->schema_id = weft_read_u64_le(p + 0u);
    out->flags = weft_read_u32_le(p + 8u);
    out->vbus_mv = weft_read_u16_le(p + 12u);
    out->temp_c = (weft_f16_t)weft_read_u16_le(p + 14u);
    out->gyro_temp_c = (weft_f16_t)weft_read_u16_le(p + 16u);
    for (uint32_t k = 0u; k < 4u; k++) {
        out->motor_rpm[k] = weft_read_u16_le(p + 20u + k * 2u);
    }
    out->crc32 = weft_read_u32_le(p + 28u);
    for (uint32_t k = 0u; k < 1u; k++) {
        out->weft_pad0[k] = weft_read_u16_le(p + 18u + k * 2u);
    }
    return WEFT_OK;
}

/// Alignment-free write to a possibly-packed destination (UMEM/DMA paths).
static inline weft_error_t weft_mcu_status_write_packed(void* buffer, size_t len, const weft_mcu_status_t* src)
{
    if (len < (size_t)WEFT_MCU_STATUS_SIZE) { return WEFT_ERR_SHORT_BUFFER; }
    uint8_t* p = (uint8_t*)buffer;
    weft_write_u64_le(p + 0u, (uint64_t)src->schema_id);
    weft_write_u32_le(p + 8u, (uint32_t)src->flags);
    weft_write_u16_le(p + 12u, (uint16_t)src->vbus_mv);
    weft_write_u16_le(p + 14u, (uint16_t)src->temp_c);
    weft_write_u16_le(p + 16u, (uint16_t)src->gyro_temp_c);
    for (uint32_t k = 0u; k < 4u; k++) {
        weft_write_u16_le(p + 20u + k * 2u, (uint16_t)src->motor_rpm[k]);
    }
    weft_write_u32_le(p + 28u, (uint32_t)src->crc32);
    for (uint32_t k = 0u; k < 1u; k++) {
        weft_write_u16_le(p + 18u + k * 2u, (uint16_t)src->weft_pad0[k]);
    }
    return WEFT_OK;
}

/// mcu_status.mode: bits [0..1] of flags
static inline uint32_t weft_mcu_status_get_mode(const weft_mcu_status_t* f)
{
    return (uint32_t)((f->flags >> 0) & 0x3u);
}
static inline void weft_mcu_status_set_mode(weft_mcu_status_t* f, uint32_t v)
{
    f->flags = (uint32_t)((f->flags & ~(0x3u << 0)) | ((v & 0x3u) << 0));
}

/// mcu_status.armed: bits [4..4] of flags
static inline uint32_t weft_mcu_status_get_armed(const weft_mcu_status_t* f)
{
    return (uint32_t)((f->flags >> 4) & 0x1u);
}
static inline void weft_mcu_status_set_armed(weft_mcu_status_t* f, uint32_t v)
{
    f->flags = (uint32_t)((f->flags & ~(0x1u << 4)) | ((v & 0x1u) << 4));
}

/// mcu_status.error_code: bits [8..15] of flags
static inline uint32_t weft_mcu_status_get_error_code(const weft_mcu_status_t* f)
{
    return (uint32_t)((f->flags >> 8) & 0xFFu);
}
static inline void weft_mcu_status_set_error_code(weft_mcu_status_t* f, uint32_t v)
{
    f->flags = (uint32_t)((f->flags & ~(0xFFu << 8)) | ((v & 0xFFu) << 8));
}
...
static inline weft_error_t weft_validate_batch_mcu_status(const void* buffer, size_t len,
                                                 size_t* out_count, size_t* out_bad_index)
{
    const size_t stride = (size_t)WEFT_MCU_STATUS_SIZE;
...
#if defined(__AVX2__)
    if (n >= 4u) {
        const __m256i want = _mm256_set1_epi64x((long long)WEFT_MCU_STATUS_SCHEMA_ID);
        for (; (i + 4u) <= n; i += 4u) {
    ...
    if (n >= 2u) {
        const uint64x2_t want = vdupq_n_u64((uint64_t)WEFT_MCU_STATUS_SCHEMA_ID);
        for (; (i + 2u) <= n; i += 2u) {
...
#endif
    for (; i < n; i++) {
        if (weft_read_u64_le(base + i * stride) != WEFT_MCU_STATUS_SCHEMA_ID) {
            if (out_count) { *out_count = i; }
            if (out_bad_index) { *out_bad_index = i; }
            return WEFT_ERR_SCHEMA_MISMATCH;
        }
