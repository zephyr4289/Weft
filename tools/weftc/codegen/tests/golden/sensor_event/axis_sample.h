// axis_sample.h — generated by weftc-codegen 0.1.0 (target: c). DO NOT EDIT.
...
#ifndef WEFT_AXIS_SAMPLE_H
#define WEFT_AXIS_SAMPLE_H

#define WEFT_AXIS_SAMPLE_SIZE 8u
#define WEFT_AXIS_SAMPLE_ALIGN 2u
#define WEFT_AXIS_SAMPLE_OFF_X 0u
#define WEFT_AXIS_SAMPLE_OFF_Y 2u
#define WEFT_AXIS_SAMPLE_OFF_Z 4u
#define WEFT_AXIS_SAMPLE_OFF_PAD 6u

#ifdef __cplusplus
extern "C" {
#endif

/// weft_axis_sample_t — 8 bytes, align 2. Offsets are normative (weft-ir),
/// synthesized padding members make the C declaration match them exactly.
/// Embedded layout type: packed 3-axis i16 sample (GPU targets refuse i16 — Law 4).
typedef struct weft_axis_sample {
    int16_t x;              // @0 (2)
    int16_t y;              // @2 (2)
    int16_t z;              // @4 (2)
    uint16_t pad;              // @6 (2)
...
static_assert(sizeof(weft_axis_sample_t) == 8, "weft ABI layout violation: axis_sample size");
static_assert(offsetof(weft_axis_sample_t, x) == 0, "weft ABI: axis_sample.x offset mismatch");
static_assert(offsetof(weft_axis_sample_t, y) == 2, "weft ABI: axis_sample.y offset mismatch");
static_assert(offsetof(weft_axis_sample_t, z) == 4, "weft ABI: axis_sample.z offset mismatch");
static_assert(offsetof(weft_axis_sample_t, pad) == 6, "weft ABI: axis_sample.pad offset mismatch");
...
static inline weft_error_t weft_axis_sample_read_packed(const void* buffer, size_t len, weft_axis_sample_t* out)
{
    if (len < (size_t)WEFT_AXIS_SAMPLE_SIZE) { return WEFT_ERR_SHORT_BUFFER; }
    const uint8_t* p = (const uint8_t*)buffer;
    out->x = (int16_t)weft_read_u16_le(p + 0u);
    out->y = (int16_t)weft_read_u16_le(p + 2u);
    out->z = (int16_t)weft_read_u16_le(p + 4u);
    out->pad = weft_read_u16_le(p + 6u);
    return WEFT_OK;
}

/// Alignment-free write to a possibly-packed destination (UMEM/DMA paths).
static inline weft_error_t weft_axis_sample_write_packed(void* buffer, size_t len, const weft_axis_sample_t* src)
{
    if (len < (size_t)WEFT_AXIS_SAMPLE_SIZE) { return WEFT_ERR_SHORT_BUFFER; }
    uint8_t* p = (uint8_t*)buffer;
    weft_write_u16_le(p + 0u, (uint16_t)src->x);
    weft_write_u16_le(p + 2u, (uint16_t)src->y);
    weft_write_u16_le(p + 4u, (uint16_t)src->z);
    weft_write_u16_le(p + 6u, (uint16_t)src->pad);
    return WEFT_OK;
}
